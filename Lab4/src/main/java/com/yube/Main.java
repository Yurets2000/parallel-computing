...
import com.yube.text.processors.SparkBasedWordCountingTextProcessor;
...
public class Main {
    ...
    public static void main(String[] args) {
        while (true) {
            try {
                String filePath = read("Enter file path:", "([a-zA-Z]:)?(\\\\[a-zA-Z0-9_.-]+)+\\\\?");
                ...
                textProcessors.add(new ClassicWordCountingTextProcessor());
                textProcessors.add(new SparkBasedWordCountingTextProcessor(ctx));
                warmUpJvm(textFile, textProcessors, 50);
                countWords(textFile, textProcessors, runsCount, isPrintResult, true);
            } catch (Exception e) {
    ...
    public static void warmUpJvm(File textFile, List<TextProcessor> textProcessors, int runsCount) {
        System.out.println("Warming up JVM...");
        countWords(textFile, textProcessors, runsCount, false, false);
        System.out.println("JVM warming finished");
    }

    public static void countWords(File textFile, List<TextProcessor> textProcessors, int runsCount, boolean isPrintResult, boolean isPrintStatistics) {
        if (isPrintResult) {
            for (TextProcessor textProcessor : textProcessors) {
                System.out.printf("Text processor: \"%s\"\n", textProcessor.getProcessorName());
                try {
                    String result = OBJECT_MAPPER.writeValueAsString(textProcessor.processText(textFile));
                    System.out.printf("Result of text processing: %s\n", result);
        ...
        if (isPrintStatistics) {
            for (TextProcessor textProcessor : textProcessors) {
                TextProcessingStatistics textProcessingStatistics = getTextProcessingStatistics(textProcessor, textFile, runsCount);
                System.out.println(textProcessingStatistics);
                System.out.println();
            }
        }
    }
